...
- The tested toolkit implementation began at immutable commit `dd54d23b481cb88e37ef61c48efbcd37c33ed856` in [draft sofa PR #1](https://github.com/kevinmartin/sofa/pull/1). The disposable consumer's initial fixture commit is `74304f4` on `main`; `go test ./...` fails at baseline only for the two intended greeting cases. Commit `02ee912` installed the manual caller. The active toolkit pin will be updated after the publisher credential change is committed.
...
- Hosted execution uses the consumer's ephemeral `GITHUB_TOKEN` for repository-scoped state writes and Copilot Requests. A separate `SOFA_PUBLISH_TOKEN`, scoped to only `sofa-disposable` with Contents and Pull requests write permission, is required in the trusted publisher job; it is not yet configured. The repository default Actions token is read-only. The repository-wide toggle that also allows Actions to approve PR reviews remains disabled. No Project or publisher credential enters the agent container; only the dedicated Copilot credential does.
