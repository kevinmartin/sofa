...
- The tested toolkit implementation is at immutable commit `d957eede266711bf374c98571332f3eb6bfc16c3` in [draft sofa PR #1](https://github.com/kevinmartin/sofa/pull/1). The disposable consumer's initial fixture commit is `74304f4` on `main`; `go test ./...` fails at baseline only for the two intended greeting cases. Commit `0725584` updated the manual caller on disposable `main`, with both reusable workflow references and `toolkit_sha` pinned to that same toolkit commit.
