...
- The two public repositories exist. The disposable consumer's initial fixture commit is `74304f4` on `main`; `go test ./...` fails at baseline only for the two intended greeting cases. The current shell's `gh` session and in-app browser are signed out, while Git SSH can fetch and push these repositories. No Project URL/ID or approved issue has been supplied yet; Kevin is choosing whether to create the disposable Project or sign in here for creation. No Copilot credential was supplied in chat or put into the toolkit.
...
The implementation is not yet installed. After the reusable workflow, CLI, consumer fixture, Project, and issue are ready, this section will identify the immutable toolkit commit, caller reference, exact commands, workflow permissions, artifact names, cancellation/rerun procedure, and recovery observation. Leave any resulting PR as a draft for Kevin to review and merge.
