...
- Present the specification digest and approval command. Approval is tied to Kevin's immutable actor identity and the canonical specification. Material changes return to Spec Review; formatting normalization cannot conceal changed acceptance criteria.
...
- [ ] **02-A:** Live Discovery produces a specification with evidence and acceptance criteria. Before Kevin approves its displayed digest, it stays out of Backlog. Approval permits Backlog but does not start implementation.
- [ ] **02-B:** Kevin's Ready transition admits the approved scope. Unauthorized moves, replayed approval for another digest, and material edits cannot admit it. Default WIP and aggregate budgets hold across split tasks.
