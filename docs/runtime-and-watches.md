...
1. **Ready admission:** current Ready status in a private Project with tightly restricted write access authorizes delivery of the issue revision frozen at admission. The Project read credential cannot set Ready, and factory automations must not do so. The GitHub API may not expose the mover for every item, so sofa does not claim actor proof from the status field. Discovery first produces a draft specification; Kevin approves that specification before the card enters Backlog. Ready remains the separate scheduling decision to build it.
