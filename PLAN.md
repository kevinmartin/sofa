...
Each downstream repository installs a small caller workflow and `.sofa.yml`. The caller handles scheduled/manual events and calls `sofa`'s reusable workflows at `@v1`. An ordinary schedule/manual invocation reconciles; a dispatch carrying a ledger attempt ID invokes the worker. Dispatch inputs identify existing admitted work and cannot authorize new work by themselves. Workflow/action inputs can select profiles or override nonsecret profile settings. Defaults live in versioned configuration on the trusted default branch.
...
3. For human-admitted delivery, verify the current Ready state and authorized transition using the Issue's `ProjectV2ItemStatusChangedEvent`. Match Project, immutable actor ID, and approved specification revision; do not confuse field creator with updater. A watch instead supplies its trusted standing policy revision, evidence, permitted change class, and budget. Missing proof requires renewed authorization; neither path may impersonate the other. [GitHub Project event schema](https://docs.github.com/en/graphql/reference/projects#projectv2itemstatuschangedevent)
...
- Discovery cannot approve its own specification; autonomous watches cannot exceed their standing scope or impersonate a human Ready event.
